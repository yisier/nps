...
| expire\_time | 到期时间，留空表示永不过期，到期后自动暂停。支持格式：`2006-01-02 15:04:05`、`2006-01-02 15:04`、`2006-01-02T15:04:05`、`2006-01-02T15:04`、`2006-01-02` |
...
| max\_conn | 客户端最大连接数量 空则为不限制 |
| max\_tunnel | 客户端最大隧道数量 空则为不限制 |
| expire\_time | 到期时间，留空表示永不过期，到期后自动暂停。格式同新增接口 |
| id | 要修改的客户端id |
