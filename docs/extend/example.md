...
**https:** 如需使用https请进行相关配置，详见 [使用https](/server/nps_extend.html#使用-https)
...
**适用范围：**  大流量传输场景，流量不经过公网服务器，但是由于p2p穿透和nat类型关系较大，不保证100%成功，支持大部分nat类型。[nat类型检测](/client/npc_extend.html#nat类型检测)
