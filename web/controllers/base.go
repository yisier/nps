...
func (s *BaseController) display(tpl ...string) {
	...
	// 传递自定义NPC客户端域名字段
	npcDomain := beego.AppConfig.String("npc_domain")
	s.Data["npcDomain"] = npcDomain

	// 传递自定义NPC初始化shell脚本字段
	npcInitShell := beego.AppConfig.String("npc_init_shell")
	s.Data["npcInitShell"] = npcInitShell

	// 传递自定义NPC装卸shell脚本字段
	npcUninstallShell := beego.AppConfig.String("npc_uninstall_shell")
	s.Data["npcUninstallShell"] = npcUninstallShell
